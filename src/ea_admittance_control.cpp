...
// MatXf pinv(MatXf x)
// {
//     Eigen::JacobiSVD<MatXf> svd(x,ComputeFullU | ComputeFullV);
...
void save_pos_to_txt(vector<array<double, 16>> &toolTobase_pos_m_sum)
{
    ...
    for (int i = 0; i < m; i++)
    {
        pos_record << toolTobase_pos_m_sum[i][3] << " " << toolTobase_pos_m_sum[i][7] << " " << toolTobase_pos_m_sum[i][11] << "\n";
    }
    pos_record.close();
...
void save_force_to_txt(vector<array<double, 6>> &tau_ext_in_base_sum)
{
    ...
    for (int i = 0; i < m; i++)
    {
        force_record << tau_ext_in_base_sum[i][1] << " " << tau_ext_in_base_sum[i][2] << " " << tau_ext_in_base_sum[i][3] << "\n";
    }
    force_record.close();
}
...
int main(int argc, char *argv[])
{
    ...
    admittance_control_callback = [&](RCI::robot::RobotState robot_state) -> JointPositions {
        ...
        cout << joint_v[0] << " " << joint_v[1] << " " << joint_v[2] << " " << joint_v[3] << " " << joint_v[4] << " " << joint_v[5] << " " << joint_v[6] << endl;
            

        // cout << "all right, all right" << endl;
        ...
        ddxe = (-external_force[0] - damp_param1*d_last_time_pos[0] - mass_param_fe*last_time_pos[0])/spring_param;
        dxe = d_last_time_pos[0] + ddxe*dt;
        xe = last_time_pos[0] + dxe*dt;

        ddye = (-external_force[1] - damp_param1*d_last_time_pos[1] - mass_param_fe*last_time_pos[1])/spring_param;
        dye = d_last_time_pos[1] + ddye*dt;
        ye = last_time_pos[1] + dye*dt;

        ddze = (-external_force[2] - damp_param2*d_last_time_pos[2] - mass_param_ne*last_time_pos[2])/spring_param;
        dze = d_last_time_pos[2] + ddze*dt;
        ze = last_time_pos[2] + dze*dt;

        ddTxe = (external_force[3] - damp_param2*d_last_time_pos[3] - mass_param_ne*last_time_pos[3])/spring_param;
        dTxe = d_last_time_pos[3] + ddTxe*dt;
        Txe = last_time_pos[3] + dTxe*dt;

        ddTye = (-external_force[4] - damp_param2*d_last_time_pos[4] - mass_param_ne*last_time_pos[4])/spring_param;
        dTye = d_last_time_pos[4] + ddTye*dt;
        Tye = last_time_pos[4] + dTye*dt;

        ddTze = (external_force[5] - damp_param2*d_last_time_pos[5] - mass_param_ne*last_time_pos[5])/spring_param;
        ...
        //std::cout << xe << " " << ye << " " << ze << " " << Txe << " " << Tye << " " << Tze << endl;
        ...
        //cout << joint_pos[0] << " " << joint_pos[1] << " " << joint_pos[2] << " " << joint_pos[3] << " " << joint_pos[4] << " " << joint_pos[5] << " " << joint_pos[6] << endl;
